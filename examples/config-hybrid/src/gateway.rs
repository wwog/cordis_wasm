...
use cordis_core::{
    ...
    EventCall, EventId, EventMode, EventReply, InjectSpec, InstanceHost, ServiceId,
};
use serde::Serialize;
...
impl ComponentFactory for HttpGatewayFactory {
    fn descriptor(&self) -> &DynamicComponentDescriptor {
        static DESCRIPTOR: std::sync::OnceLock<DynamicComponentDescriptor> =
    ...
    fn instantiate(&self, host: InstanceHost) -> ComponentFuture<'_, Box<dyn ComponentInstance>> {
        Box::pin(
            async move { Ok(Box::new(HttpGatewayInstance { host }) as Box<dyn ComponentInstance>) },
        )
    }
}
...
impl ComponentInstance for HttpGatewayInstance {
    ...
    fn call_service(&mut self, call: DynamicCall) -> ComponentFuture<'_, Vec<u8>> {
        Box::pin(async move {
            ...
            let payload = cordis_core::encode_service_payload(&record).map_err(|error| {
